/***************************************************************************
    begin       : Sun March 23 2023
    copyright   : (C) 2023 by Reto Schneider
    email       : code@reto-schneider.ch

 ***************************************************************************
 *          Please see toplevel file COPYING for license details           *
 ***************************************************************************/

static void _import_053_001_04_read_account_spec(AB_IMEXPORTER *ie,
...
static int
_import_053_001_04_read_balance(AB_IMEXPORTER *ie, GWEN_XMLNODE *xmlNode,
  ...
  /* read amount */
  n = GWEN_XMLNode_GetNodeByXPath(xmlNode, "Amt",
  ...
  if (s && *s) {
    if (strcasecmp(s, "CLBD") == 0 || /* CLBD: Closing Booked Balance */
        strcasecmp(s, "PRCD") ==
            0) { /* PRCD: Previously Closed Booked Balance */
      ...
      DBG_ERROR(AQBANKING_LOGDOMAIN,
                "Unknown balance type [%s] in <BAL>, ignoring", s);
      AB_Balance_free(balance);
    }
  }
...
static int
_import_053_001_04_read_balances(AB_IMEXPORTER *ie, GWEN_XMLNODE *xmlNode,
                                 AB_IMEXPORTER_ACCOUNTINFO *accountInfo) {
  GWEN_XMLNODE *n;
  ...
  while (n) {
    ...
    rv = _import_053_001_04_read_balance(ie, n, accountInfo);
    if (rv < 0) {
...
static int _import_053_001_04_read_transaction_details(AB_IMEXPORTER *ie,
                                                       GWEN_XMLNODE *xmlNode,
                                                       AB_TRANSACTION *t,
                                                       int isCredit) {
  const char *s;
  ...
  if (isCredit) {
    ...
    s = GWEN_XMLNode_GetCharValueByPath(xmlNode, "RltdPties/DbtrAcct/Id/IBAN",
                                        NULL);
    if (s && *s)
    ...
    s = GWEN_XMLNode_GetCharValueByPath(xmlNode, "RltdPties/CdtrAcct/Id/IBAN",
                                        NULL);
    if (s && *s)
    ...
    s = GWEN_XMLNode_GetCharValueByPath(
        xmlNode, "RltdPties/Cdtr/Id/PrvtId/Othr/Id", NULL);
    if (s && *s)
  ...
  if (s && *s) {
    ...
    s = GWEN_XMLNode_GetCharValueByPath(xmlNode, "BkTxCd/Domn/Fmly/SubFmlyCd",
                                        NULL);
    if (s && *s)
      GWEN_Buffer_AppendString(tbuf, s);
...
static int
_import_053_001_04_read_transaction(AB_IMEXPORTER *ie, GWEN_XMLNODE *xmlNode,
                                    AB_IMEXPORTER_ACCOUNTINFO *accountInfo) {
  const char *s;
  GWEN_XMLNODE *n;
  AB_TRANSACTION *t;
  ...
  n = GWEN_XMLNode_GetNodeByXPath(xmlNode, "NtryDtls/TxDtls",
                                  GWEN_PATH_FLAGS_NAMEMUSTEXIST);
  if (n) {
    int rv;

    rv = _import_053_001_04_read_transaction_details(ie, n, t, isCredit);
    if (rv < 0) {
      DBG_INFO(AQBANKING_LOGDOMAIN, "here (%d)", rv);
      return rv;
    }
  ...
  /* check transaction */
  if (!(AB_Transaction_GetValue(t) &&
        (AB_Transaction_GetDate(t) || AB_Transaction_GetValutaDate(t)))) {
...
static int
_import_053_001_04_read_transactions(AB_IMEXPORTER *ie, GWEN_XMLNODE *xmlNode,
                                     AB_IMEXPORTER_ACCOUNTINFO *accountInfo) {
  GWEN_XMLNODE *n;
  ...
  while (n) {
    ...
    rv = _import_053_001_04_read_transaction(ie, n, accountInfo);
    if (rv < 0) {
      DBG_INFO(AQBANKING_LOGDOMAIN, "here (%d)", rv);
      return rv;
    }
...
static int _import_053_001_04_statement(AB_IMEXPORTER *ie,
  ...
  if (n) {
    ...
    _import_053_001_04_read_account_spec(ie, n, accountSpec);
    accountInfo = AB_ImExporterContext_GetOrAddAccountInfo(
        ctx, 0, AB_AccountSpec_GetIban(accountSpec),
        AB_AccountSpec_GetBankCode(accountSpec),
        AB_AccountSpec_GetAccountNumber(accountSpec), AB_AccountType_Unknown);
    assert(accountInfo);
    AB_AccountSpec_free(accountSpec);
  }

  /* read balances */
  rv = _import_053_001_04_read_balances(ie, xmlNode, accountInfo);
  if (rv < 0) {
    DBG_INFO(AQBANKING_LOGDOMAIN, "here (%d)", rv);
    return rv;
  }
  ...
  /* read transactions */
  rv = _import_053_001_04_read_transactions(ie, xmlNode, accountInfo);
  if (rv < 0) {
    DBG_INFO(AQBANKING_LOGDOMAIN, "here (%d)", rv);
    return rv;
  }
...
int AH_ImExporterCAMT_Import_053_001_04(AB_IMEXPORTER *ie,
                                        AB_IMEXPORTER_CONTEXT *ctx,
                                        GWEN_DB_NODE *params,
                                        GWEN_XMLNODE *xmlNode) {
  GWEN_XMLNODE *n;
